...
import asyncio
from anyio import to_thread  # NEW: 把同步编码丢到线程池
...
parser.add_argument("--model-path", "-m",
                    default="/root/autodl-tmp/tenxun/FireRedASR-AED-L",
                    help="模型目录/权重路径")
...
parser.add_argument("--slots", type=int, default=16,help="KV池最大并发序列数（SLOTS）")
parser.add_argument("--cap", type=int, default=64, help="每序列最大解码步（CAP）")
...
# NEW: 编码并发限制（避免事件循环饥饿 & 保护GPU/CPU）
ENC_CONCURRENCY = int(os.getenv("ENC_CONCURRENCY", "3"))
_encoder_sema = asyncio.Semaphore(ENC_CONCURRENCY)
...
# NEW: 线程池中的编码 + cross K/V 打包（受信号量限流）
async def encode_and_pack(wav_path: str):
    async with _encoder_sema:
        t0 = time.time()
        enc_out, src_mask = await to_thread.run_sync(_run_encoder_strict, asr, wav_path)
        enc_state = await to_thread.run_sync(build_enc_state_for_request, decoder, enc_out, src_mask)
        t1 = time.time()
        print(f"[encode] path={os.path.basename(wav_path)} done in {t1 - t0:.3f}s")
        return enc_state
...
async def _startup():
    ...
    _device = torch.device(
        args.device if args.device != "cuda"
    ...
    print(f"[Init] KVCache: SLOTS={spec.slots}, CAP={spec.cap}, H={spec.n_heads}, "
          f"D={spec.head_dim}, dtype={spec.dtype}, device={spec.device}")
    ...
    kv = kv_mod
    runner = run
    print(f"[Init] Runner ready. Continuous batching enabled. ENC_CONCURRENCY={ENC_CONCURRENCY}")
...
async def health():
    ...
    return {
        ...
        "enc_concurrency": ENC_CONCURRENCY,  # NEW: 暴露当前编码并发
    }
...
async def asr_single(
    ...
    try:
        ...
        # NEW: 线程池里做编码 + cross K/V 打包
        enc_state = await encode_and_pack(wav_path)
        ...
        print(f"[route] before await fut t={time.time():.3f}")
        result = await fut
        print(f"[route] after  await fut t={time.time():.3f}")

        ids: List[int] = result["tokens"]
        text = asr.tokenizer.detokenize(ids) if hasattr(asr.tokenizer, "detokenize") else (
            asr.tokenizer.decode(ids) if hasattr(asr.tokenizer, "decode")
...
async def asr_batch(
    ...
    try:
        ...
        for uf in audio_files:
            with tempfile.NamedTemporaryFile(
                delete=False,
                suffix=os.path.splitext(uf.filename or "")[-1] or ".wav"
            ) as tf:
        ...
        # NEW: 并发编码（受 _encoder_sema 限制），随后批量 submit
        enc_states = await asyncio.gather(*(encode_and_pack(p) for p in temps))

        futs = []
        for (uf, enc_state) in zip(audio_files, enc_states):
            fut = await runner.submit(
                enc=enc_state,
                max_len=cfg.max_len,
                bos_id=bos,
