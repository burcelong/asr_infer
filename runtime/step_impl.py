...
class Stepper:
    ...
    def __call__(
        ...
        _K_ignored: Tensor, _V_ignored: Tensor, _L_ignored: Tensor,
        ...
        for layer_idx, layer in enumerate(self.dec.layer_stack):
            if can_fast:
                ...
                Kc, Vc, Lc = self.kv.get_batch_fixedcap_layer(layer_idx, sids)
            ...
            out = _flash_attn_with_kvcache(
                q=q, k_cache=Kc, v_cache=Vc, k=k, v=v,
            ...
            for r in batch_reqs:
                ext = r.enc.extra
                k_unpad_layers: List[Tensor] = ext["cross_k_unpad_layers"]
                v_unpad_layers: List[Tensor] = ext["cross_v_unpad_layers"]
                k_i = k_unpad_layers[layer_idx]     # [T_i, H, D]
                v_i = v_unpad_layers[layer_idx]
                k_parts.append(k_i)
                v_parts.append(v_i)
                lens.append(k_i.size(0))

            attn_dtype = self.attn_dtype
            enc_k_cat = torch.cat(k_parts, dim=0).to(attn_dtype) if k_parts else \
                        torch.empty(0, self.n_head, self.d_k, dtype=attn_dtype, device=device)
            enc_v_cat = torch.cat(v_parts, dim=0).to(attn_dtype) if v_parts else enc_k_cat.clone()
            ...
            if lens:
                ...
                max_seqlen_k = int(lens_t.max().item())
            else:
                cu_k = torch.zeros(B + 1, device=device, dtype=torch.int32)
                max_seqlen_k = 0
            ...
            x_cross = layer.cross_attn.varlen_cross_attention(
                q=x_cross, k_unpad=enc_k_cat, v_unpad=enc_v_cat,
...
def _flash_attn_with_kvcache(
    q: Tensor, k_cache: Tensor, v_cache: Tensor, k: Tensor, v: Tensor,
    rotary_cos: Tensor, rotary_sin: Tensor, cache_seqlens: Tensor,
    ...
    out = _fa_kvcache(
        ...
        softmax_scale=softmax_scale, causal=causal, rotary_interleaved=True,
    )
    return out
